...
if len(df) > 0:
    ...
    for c in ['ann_date', 'roe', 'gross_margin', 'net_margin', 'pe', 'pb', 'ps', 'total_revenue', 'net_profit']:
        if c in d.index and d[c] is not None:
            print(f'    {c}: {d[c]}')

# 4. 指数 K 线
print('\n【4】指数 K 线 (000001.SH 上证指数)')
...
print(f'  ✅ {len(df)} 条')
cols7 = list(df.columns)
for _, r in df.iterrows():
    print(f'    {r[cols7[0]]} {r[cols7[1]]}')
...
print('=' * 60)
print()
print('✅ 可用: 股票 K 线、复权因子、财务指标、指数数据、')
print('         资金流向、基金数据、交易日、股票基础')
print('⚠️ 期货接口: 需要更高积分等级')
print()
print('Token 权限总结:')
print('  - 股票核心数据: 全部可用')
print('  - 指数数据: 全部可用')
print('  - 财务数据: 基本可用 (fina_indicator)')
print('  - 资金流向: 可用')
print('  - 期货数据: 不可用 (需升级积分)')
